...
Private Sub ProcessValidationTarget( _
    ...
    If Not AV_DataAccess.WorksheetExists(target.TableName) Then
        AV_UI.AppendUserLog "ERROR: Sheet '" & target.TableName & "' not found. Skipping."
        Exit Sub
    End If
    
    Set wsTarget = ThisWorkbook.Sheets(target.TableName)
    
    ' Get target table (first ListObject in sheet)
    If wsTarget.ListObjects.Count = 0 Then
        AV_UI.AppendUserLog "ERROR: No table found in sheet '" & target.TableName & "'. Skipping."
        Exit Sub
    End If
    
    Set tblTarget = wsTarget.ListObjects(1)
    AV_UI.AppendUserLog "Table: " & tblTarget.Name & " (Rows: " & tblTarget.ListRows.Count & ")"
