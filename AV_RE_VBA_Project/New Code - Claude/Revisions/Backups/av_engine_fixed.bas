...
ValidationError:
    AV_Core.DebugMessage "[ValidateSingleRow] Error: Row " & rowNum & " ? Validating column '" & colLetter & "' using function: " & funcName, MODULE_NAME
    AV_UI.AppendUserLog "Error during validation: Row " & rowNum & " ? Validating column '" & colLetter & "' using function: " & funcName
    Resume Next
End Sub
...
Public Sub RunAutoCheckDataValidation(wsConfig As Worksheet, _
    ...
    AV_Core.DebugMessage "[RunAutoCheckDataValidation] Starting validation on " & totalValid & " rows.", MODULE_NAME
...
SkipRow:
    ...
    AV_Core.DebugMessage "[RunAutoCheckDataValidation] Progress: " & progressCount & " / " & totalValid, MODULE_NAME
    AV_Core.DebugMessage "[RunAutoCheckDataValidation] Completed.", MODULE_NAME
