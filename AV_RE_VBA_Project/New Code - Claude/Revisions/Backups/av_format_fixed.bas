...
Public Function DefaultFormatMap() As Object
    ...
    AV_Core.DebugMessage "Format Map was loaded from default settings. Loaded from: '" & "Config" & "' Workbook Sheet", "DefaultFormatMap"
...
Private Sub SetReviewStatus(ReviewRequired As Boolean, rowRange As Range, Optional AutoCorrected As Boolean = False)
    ...
    If tbl Is Nothing Then
        AV_Core.DebugMessage "?? Table '" & GlobalFormatTableName & "' not found on " & wsConfig.Name, "SetReviewStatus"
        Exit Sub
    End If
...
Public Sub setFormat(TargetCell As Range, FormatType As String, FormatMap As Object)
    ...
    Else
        AV_Core.DebugMessage "FormatMap type: '" & FormatType & "' could not be found in the '" & GlobalFormatTableName & "' Config table", "setFormat"
    End If
End Sub
