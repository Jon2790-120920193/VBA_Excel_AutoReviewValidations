...
' VERSION: 2.51 - With version tracking & enhanced diagnostics
' Patched (V +0.01) with Runautocheckdatavalidation optimized
'   Added WriteSimpleTag Helper function
'   Attempt to improve system efficiency/reduce runtime
...
Public Sub RunFullValidationMaster(Optional ByVal sheetName As String = "", Optional ByVal english As Boolean = True)
    ...
    ' NEW in v2.5: Set bulk validation flag to prevent Worksheet_Change cascade
...
Cleanup:
    ' NEW in v2.5: Clear bulk validation flag
...
' ======================================================
' PERFORMANCE FIX: RunAutoCheckDataValidation
' Replace the existing function in AV_Engine with this
' ======================================================

' ======================================================
' RUN AUTO CHECK DATA VALIDATION (OPTIMIZED v2.5.1)
' ======================================================
Public Sub RunAutoCheckDataValidation(wsConfig As Worksheet, _
    ...
    ' Performance: Cache column indices at start
    Dim colIndices As Object
    Set colIndices = CreateObject("Scripting.Dictionary")
    Dim dropColIndices As Object
    Set dropColIndices = CreateObject("Scripting.Dictionary")
    ...
    If colMetaDict Is Nothing Or colMetaDict.Count = 0 Then
        AV_UI.AppendUserLog "Simple validation: No columns configured, skipping"
        Exit Sub
    End If

    totalValid = UBound(keyRows) - LBound(keyRows) + 1
    AV_Core.DebugMessage "Simple validation starting: " & totalValid & " rows, " & colMetaDict.Count & " columns", MODULE_NAME
    AV_UI.AppendUserLog "Simple validation: " & totalValid & " rows, " & colMetaDict.Count & " columns"

    ' ======================================================
    ' PERFORMANCE: Pre-cache all column indices
    ' This avoids repeated ListColumns lookups per row
    ' ======================================================
    For Each colKey In colMetaDict.Keys
        Set meta = colMetaDict(colKey)
        
        Dim reviewHeader As String
        reviewHeader = ""
        If meta.Exists("ReviewLetter") Then reviewHeader = CStr(meta("ReviewLetter"))
        
        If Len(reviewHeader) > 0 Then
            ' Find column index for data column
            Dim dataColIdx As Long
            dataColIdx = 0
            On Error Resume Next
            dataColIdx = mCurrentTargetTable.ListColumns(reviewHeader).Index
            On Error GoTo 0
            
            If dataColIdx > 0 Then
                colIndices(colKey) = dataColIdx
            Else
                AV_Core.DebugMessage "Column not found in table: " & reviewHeader, MODULE_NAME
            End If
            
            ' Find column index for drop column (comment column)
            Dim dropHeader As String
            dropHeader = ""
            If meta.Exists("CommentDropCol") Then dropHeader = CStr(meta("CommentDropCol"))
            
            If Len(dropHeader) > 0 And Not dropColIndices.Exists(dropHeader) Then
                Dim dropColIdx As Long
                dropColIdx = 0
                On Error Resume Next
                dropColIdx = mCurrentTargetTable.ListColumns(dropHeader).Index
                On Error GoTo 0
                
                If dropColIdx > 0 Then
                    dropColIndices(dropHeader) = dropColIdx
                End If
            End If
        End If
    Next colKey
    
    AV_Core.DebugMessage "Cached " & colIndices.Count & " data columns, " & dropColIndices.Count & " drop columns", MODULE_NAME

    ' ======================================================
    ' PERFORMANCE: Disable events and screen updating ONCE
    ' ======================================================
    Application.EnableEvents = False
    Application.ScreenUpdating = False
    ...
    For i = LBound(keyRows) To UBound(keyRows)
        ...
        ' Calculate table-relative row index
        Dim tableRowIdx As Long
        tableRowIdx = rowNum - mCurrentTargetTable.DataBodyRange.Row + 1
        
        If tableRowIdx < 1 Or tableRowIdx > mCurrentTargetTable.ListRows.Count Then GoTo SkipRow
        
        Set dropColMsgs = CreateObject("Scripting.Dictionary")
        
        ' ======================================================
        ' COLUMN LOOP - Use cached indices
        ' ======================================================
        For Each colKey In colIndices.Keys
            Set meta = colMetaDict(colKey)
            
            Dim dataColIndex As Long
            dataColIndex = colIndices(colKey)
            
            ' Get cell value directly using cached index
            cellValue = ""
            On Error Resume Next
            cellValue = Trim(CStr(mCurrentTargetTable.DataBodyRange.Cells(tableRowIdx, dataColIndex).value))
            On Error GoTo 0
            ...
            found = False
            If meta.Exists("ValidColumnListEN") Then
                If IsArray(meta("ValidColumnListEN")) Then
                    found = ExistsInArray(meta("ValidColumnListEN"), cellValue)
                End If
            End If
            If Not found And meta.Exists("ValidColumnListFR") Then
                If IsArray(meta("ValidColumnListFR")) Then
                    found = ExistsInArray(meta("ValidColumnListFR"), cellValue)
                End If
            End If

            Dim dropHeader2 As String
            dropHeader2 = ""
            If meta.Exists("CommentDropCol") Then dropHeader2 = CStr(meta("CommentDropCol"))
            
            If Not found Then
                ' Build error message
                Dim errorMsg As String
                Dim colDisplayName As String
                colDisplayName = CStr(colKey)
                If english And meta.Exists("ColumnNameEN") Then
                    colDisplayName = CStr(meta("ColumnNameEN"))
                ElseIf Not english And meta.Exists("ColumnNameFR") Then
                    colDisplayName = CStr(meta("ColumnNameFR"))
                End If
                
                errorMsg = colDisplayName & " - Invalid value '" & cellValue & "'"

                If Len(dropHeader2) > 0 Then
                    If Not dropColMsgs.Exists(dropHeader2) Then
                        Set dropColMsgs(dropHeader2) = CreateObject("Scripting.Dictionary")
                    End If
                    
                    msgArr(1) = CStr(colKey)  ' Column header for tag ID
                    msgArr(2) = errorMsg
                    msgArr(3) = "Error"
                    dropColMsgs(dropHeader2).Add dropColMsgs(dropHeader2).Count + 1, msgArr
                    
                    ' Apply error formatting to data cell
                    On Error Resume Next
                    AV_Format.setFormat mCurrentTargetTable.DataBodyRange.Cells(tableRowIdx, dataColIndex), "Error", FormatMap
                    On Error GoTo 0
                End If
            Else
                ' Value is valid - clear any existing error format
                On Error Resume Next
                Dim currentFmt As String
                currentFmt = AV_Format.getFormatType(mCurrentTargetTable.DataBodyRange.Cells(tableRowIdx, dataColIndex), FormatMap)
                If currentFmt = "Error" Then
                    AV_Format.setFormat mCurrentTargetTable.DataBodyRange.Cells(tableRowIdx, dataColIndex), "Default", FormatMap
                    
                    ' Also need to clear tag
                    If Len(dropHeader2) > 0 Then
                        If Not dropColMsgs.Exists(dropHeader2) Then
                            Set dropColMsgs(dropHeader2) = CreateObject("Scripting.Dictionary")
                        End If
                        msgArr(1) = CStr(colKey)
                        msgArr(2) = ""
                        msgArr(3) = "Default"
                        dropColMsgs(dropHeader2).Add dropColMsgs(dropHeader2).Count + 1, msgArr
                    End If
...
SkipCol:
        ...
        For Each dropColKey In dropColMsgs.Keys
            If dropColIndices.Exists(CStr(dropColKey)) Then
                Dim dropIdx As Long
                dropIdx = dropColIndices(CStr(dropColKey))
                Dim dropCell As Range
                Set dropCell = mCurrentTargetTable.DataBodyRange.Cells(tableRowIdx, dropIdx)
                
                For Each cMsg In dropColMsgs(dropColKey).Items
                    DCMsgTxt = cMsg(2)
                    cMsgErrorType = CStr(cMsg(3))
                    
                    ' Write tag directly (simplified)
                    WriteSimpleTag dropCell, CStr(cMsg(1)), DCMsgTxt, cMsgErrorType
                Next cMsg
            End If
        Next dropColKey
        ...
        Set keyCell = mCurrentTargetTable.DataBodyRange.Cells(tableRowIdx, keyColNum)
        
        ' Simple approach: check if any error messages were written
        Dim hasErrors As Boolean
        hasErrors = False
        For Each dropColKey In dropColMsgs.Keys
            Dim msgDict As Object
            Set msgDict = dropColMsgs(dropColKey)
            Dim msgItem As Variant
            For Each msgItem In msgDict.Items
                If CStr(msgItem(3)) = "Error" Then
                    hasErrors = True
                    Exit For
                End If
            Next msgItem
        ...
        If hasErrors Then
            AV_Format.setFormat keyCell, "Error", FormatMap
        End If
        
        progressCount = progressCount + 1
        
        ' Progress updates
        If progressCount Mod 10 = 0 Then
            DoEvents  ' Allow UI updates
        End If
        If progressCount Mod 25 = 0 Then
            AV_UI.AppendUserLog "Simple validation: " & progressCount & " / " & totalValid & " (" & Format((Timer - startTime), "0.0") & "s)"
        End If
...
SkipRow:
    ...
    AV_Core.DebugMessage "Simple validation complete: " & progressCount & " rows in " & Format(elapsed, "0.00") & "s", MODULE_NAME
    AV_UI.AppendUserLog "Simple validation complete: " & progressCount & " rows in " & Format(elapsed, "0.0") & "s"
...
' ======================================================
' HELPER: Write simple tag to cell (optimized)
' ======================================================
Private Sub WriteSimpleTag(dropCell As Range, tagId As String, msgText As String, formatType As String)
    If dropCell Is Nothing Then Exit Sub
    
    Dim tagStart As String, tagEnd As String
    tagStart = "[[SYS_TAG Col " & tagId & ":"
    tagEnd = "]]"
    
    Dim existingText As String
    existingText = CStr(dropCell.value)
    
    ' Remove existing tag for this column if present
    Dim sPos As Long, ePos As Long
    sPos = InStr(1, existingText, tagStart, vbTextCompare)
    If sPos > 0 Then
        ePos = InStr(sPos, existingText, tagEnd, vbTextCompare)
        If ePos > 0 Then
            existingText = Left(existingText, sPos - 1) & Mid(existingText, ePos + Len(tagEnd))
        End If
    End If
    
    ' Clean up whitespace
    existingText = Trim(Replace(existingText, vbCr, ""))
    Do While InStr(existingText, vbLf & vbLf) > 0
        existingText = Replace(existingText, vbLf & vbLf, vbLf)
    Loop
    If Left(existingText, 1) = vbLf Then existingText = Mid(existingText, 2)
    If Right(existingText, 1) = vbLf Then existingText = Left(existingText, Len(existingText) - 1)
    
    ' Add new tag if not "Default" (clearing)
    If formatType <> "Default" And Len(msgText) > 0 Then
        Dim newTag As String
        newTag = "[[SYS_TAG Col " & tagId & ": " & msgText & " ]]"
        
        If Len(existingText) > 0 Then
            existingText = existingText & vbLf & newTag
        Else
            existingText = newTag
        End If
    End If
    
    dropCell.value = existingText
End Sub
