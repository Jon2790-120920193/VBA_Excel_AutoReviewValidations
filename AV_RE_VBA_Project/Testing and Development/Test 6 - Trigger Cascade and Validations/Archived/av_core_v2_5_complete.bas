...
' MERGED: av_core_fixed + av_core_enhanced_table + v2.5 additions
' ======================================================
...
Public Function GetAutoValidationMap(Optional wsConfig As Worksheet) As Object
    ...
    If tbl Is Nothing Then
        DebugMessage "Table not found: " & MAPPING_TABLE_NAME, MODULE_NAME
...
Public Sub ClearTableCache()
    ...
    DebugMessage "Validation table cache cleared", MODULE_NAME
End Sub
...
Public Function GetValidationTable(tableName As String) As ListObject
    ...
    If Not gTableCache.IsLoaded Then
        ...
        DebugMessage "Validation table cache loaded", MODULE_NAME
    End If
    
    ' Return from cache based on table name
    ...
    If GetValidationTable Is Nothing Then
        DebugMessage "Table not found: " & tableName, MODULE_NAME
    End If
End Function
...
Public Function ShouldValidateRow(ByVal rowNum As Long, wsTarget As Worksheet, Optional ByVal ForceValidation As Boolean = False) As Boolean
    ...
    If tbl Is Nothing Then
        ShouldValidateRow = True  ' Default to validate if no ForceValidationTable
        Exit Function
    End If
...
' ======================================================
' COLUMN METADATA (for legacy compatibility)
' ======================================================
...
Public Function GetDDMValidationColumns(wsConfig As Worksheet) As Object
    ...
    For Each r In DDMRefTable.ListRows
        ...
        If StrComp(autoCheckVal, "TRUE", vbTextCompare) = 0 Then
            ...
            item("ReviewLetter") = reviewColName
...
Private Function GetNonEmptyRangeInColumn(sheetName As String, colHeader As String, startRow As Long, endRow As Long) As Range
    ...
    ' Try to find column by header name in table first
