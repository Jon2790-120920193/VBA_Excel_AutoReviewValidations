...
Public Function GetAutoValidationMap(Optional wsConfig As Worksheet) As Object
    ...
    If Not gAutoValidationMap Is Nothing Then
        Debug.Print "[GetAutoValidationMap] Using CACHED version (" & gAutoValidationMap.Count & " items)"
        DoEvents
        Set GetAutoValidationMap = gAutoValidationMap
        Exit Function
    End If
    
    Debug.Print "[GetAutoValidationMap] NOT CACHED - Loading fresh..."
    DoEvents
    ...
    Debug.Print "[GetAutoValidationMap] Starting..."
    DoEvents
    ...
    If tbl Is Nothing Then
        Debug.Print "[GetAutoValidationMap] ERROR: AutoValidationCommentPrefixMappingTable not found"
        Set GetAutoValidationMap = dict
        Exit Function
    End If
    
    Debug.Print "[GetAutoValidationMap] Table found: " & tbl.Name
    Debug.Print "[GetAutoValidationMap] Rows to process: " & tbl.ListRows.Count
    Debug.Print "[GetAutoValidationMap] Starting row loop..."
    DoEvents
    ...
    For Each r In tbl.ListRows
        ...
        Debug.Print "[Row " & rowIndex & "] Processing..."
        DoEvents
        ...
        If Err.Number <> 0 Then
            Debug.Print "[Row " & rowIndex & "] ERROR getting column indices: " & Err.Description
            DoEvents
            Err.Clear
        ...
        ' Read Dev Function Names using DataBodyRange
        devFunc = "Validate_Column_" & Trim(CStr(tbl.DataBodyRange(rowIndex, colDevFunc).Value))
        
        If Err.Number <> 0 Then
            Debug.Print "[Row " & rowIndex & "] ERROR reading Dev Function Names: " & Err.Description
            DoEvents
            Err.Clear
            skipCount = skipCount + 1
            GoTo NextRow
        ...
        Debug.Print "[Row " & rowIndex & "] devFunc = " & devFunc
        DoEvents
        
        ' Check if empty
        If devFunc = "Validate_Column_" Then
            Debug.Print "[Row " & rowIndex & "] SKIPPED - Dev Function Name is empty"
            DoEvents
            skipCount = skipCount + 1
            GoTo NextRow
        End If
        ...
        ' Read all columns using DataBodyRange
        ...
        If Err.Number <> 0 Then
            Debug.Print "[Row " & rowIndex & "] ERROR reading data: " & Err.Description
            DoEvents
            Err.Clear
            skipCount = skipCount + 1
            GoTo NextRow
        ...
        ' Add to dictionary
        Debug.Print "[Row " & rowIndex & "] Adding " & devFunc & " to dictionary"
        DoEvents
        
        Set dict(devFunc) = item
        successCount = successCount + 1
        
        Debug.Print "[Row " & rowIndex & "] SUCCESS - Total loaded: " & successCount
        DoEvents
        
NextRow:
    Next r
    
    Debug.Print "[GetAutoValidationMap] Loop complete"
    Debug.Print "[GetAutoValidationMap] Success: " & successCount & " | Skipped: " & skipCount
    DoEvents
...
ErrorHandler:
    Debug.Print "[GetAutoValidationMap] CRITICAL ERROR"
    Debug.Print "Error #" & Err.Number & ": " & Err.Description
    Debug.Print "Row being processed: " & rowIndex
    DoEvents
    Set GetAutoValidationMap = dict
End Function
